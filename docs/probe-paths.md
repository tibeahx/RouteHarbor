...
## Selective dispatcher and comparative checks

Selective profiles use a separate pinned sing-box dispatcher, generated by the
helper from typed settings. Prepared local SOCKS inputs carry proxy destinations;
reserved marked sockets select tunnel and DPI paths with closed egress on failure.
The direct source is excluded from bypass selection unless it is a carrier to the
configured private relay. Ordinary destinations leave directly through WAN.

The restriction detector compares the same public addresses, IP families, SNI and
verified certificate on direct and the actual bypass exit, including relay egress
when continuity is enabled. It uses TCP/TLS port 443 with no application request
replay and shares the configured probe concurrency. The existing HTTP source-health
measurements remain separate from evidence of a domain restriction.
See [selective-routing.md](selective-routing.md) for confirmation and expiry rules.
