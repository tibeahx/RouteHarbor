// OpenRHP is an unprivileged OpenWrt network controller with an embedded UI.
package main
...
func run(args []string) error {
	if len(args) == 0 {
		return errors.New(
			"usage: openrhp bootstrap|serve|doctor|validate|api|backup|token|as-service|version",
		)
	}
	switch args[0] {
	case "version":
		fmt.Println("OpenRHP " + version)
		return nil
	case "bootstrap":
...
func bootstrap(args []string) error {
	...
	state := f.String("state", "/etc/openrhp", "private service state directory")
	...
	fmt.Printf(
		"OpenRHP initialized. Credential written to %s (0600). Traffic routing is off.\n",
		*out,
	)
	return nil
}
...
func serve(args []string) error {
	...
	state := f.String("state", "/etc/openrhp", "private state directory")
	runtimeDir := f.String(
		"runtime",
		"/var/run/openrhp-engines",
	...
	fmt.Printf("OpenRHP listening on %s; credentials are required\n", *listen)
	select {
	case err := <-done:
