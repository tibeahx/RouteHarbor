...
func asService(args []string) error {
	if len(args) == 0 || (args[0] != "token" && args[0] != "backup") {
		return errors.New(
			"usage: openrhp as-service token|backup [flags]; only local credential and backup administration is permitted",
		)
	}
	if os.Geteuid() != 0 {
		return errors.New(
...
func checkAdministrationOwner(state, command string) error {
	...
	if stat, ok := info.Sys().(*syscall.Stat_t); ok && os.Geteuid() == 0 && stat.Uid != 0 {
		return fmt.Errorf(
			"service-owned state must be administered without root file access; use openrhp as-service %s with the same flags and a new output file under /etc/openrhp/admin",
			command,
		)
	}
	return nil
