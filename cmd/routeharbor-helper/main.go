// routeharbor-helper is an explicitly installed privileged service. The unprivileged
...
func run() error {
	if len(os.Args) < 2 {
		return errors.New("usage: routeharbor-helper serve|watchdog|recover [options]")
	}
	...
	if os.Args[1] == "verify-wireless" {
		return wireless.NewVerifier("/etc/routeharbor-helper", "gateway", "/etc/routeharbor/nodes").
	...
	fs := flag.NewFlagSet("routeharbor-helper "+os.Args[1], flag.ContinueOnError)
	dir := fs.String("state-dir", "/etc/routeharbor-helper", "private durable journal directory")
	...
	identityDir := fs.String(
		"identity-dir",
		"/etc/routeharbor/nodes",
		"gateway paired-node identity directory",
	)
	socket := fs.String("socket", "/var/run/routeharbor/helper.sock", "local helper socket")
