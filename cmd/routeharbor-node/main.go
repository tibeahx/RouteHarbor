...
func run(args []string) error {
	if len(args) == 0 {
		return errors.New(
			"usage: routeharbor-node bootstrap|serve|helper|watchdog|verify-wireless [options]",
		)
	}
	if args[0] == "verify-wireless" {
		return wireless.NewVerifier("/etc/routeharbor-node-helper", "node", "/etc/routeharbor-node").
	...
	case "helper", "watchdog":
		...
		verifier := wireless.NewVerifier(dir, "node", "/etc/routeharbor-node")
		backend.Capabilities = func(ctx context.Context) node.Capabilities {
			return node.VerifiedCapabilities(ctx, verifier, "/etc/routeharbor-node")
		}
	...
	case "version":
		return json.NewEncoder(os.Stdout).
			Encode(map[string]string{"project": "RouteHarbor", "role": "node", "protocol": "1", "uid": strconv.Itoa(os.Geteuid())})
	default:
