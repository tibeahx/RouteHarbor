// routeharbor-relay runs an explicitly paired private relay. It never configures
...
func run(ctx context.Context, args []string, out io.Writer) error {
	if len(args) == 0 || args[0] != "identity" && args[0] != "serve" {
		return errors.New(
			"usage: routeharbor-relay identity|serve --state /private/state [--config /private/config.json]",
		)
	}
	flags := flag.NewFlagSet("routeharbor-relay", flag.ContinueOnError)
	flags.SetOutput(io.Discard)
	state := flags.String(
		"state",
		"/var/lib/routeharbor-relay",
	...
	if args[0] == "serve" {
		flags.StringVar(
			&configPath,
			"config",
			"/etc/routeharbor-relay/config.json",
