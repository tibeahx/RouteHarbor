// routeharbor-release signs or verifies release manifests offline. It never installs
...
func run(args []string) error {
	if len(args) == 0 {
		return errors.New("usage: routeharbor-release keygen|sign|verify")
	}
	...
	case "verify":
		...
		fmt.Printf(
			"Verified RouteHarbor %s, commit %s, architecture %s\n",
			m.Version,
			m.Commit,
			*arch,
		)
