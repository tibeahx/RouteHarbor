...
for spec in tests/browser/*.spec.js; do
  ...
  "$browser_tmp/openrhp" bootstrap --state "$suite_tmp/state" --token-file "$suite_tmp/admin.token"
  "$browser_tmp/openrhp" serve --state "$suite_tmp/state" --runtime "$suite_tmp/runtime" --listen 127.0.0.1:8787 > "$suite_tmp/server.log" 2>&1 &
  server_pid=$!
  export OPENRHP_TEST_TOKEN_FILE="$suite_tmp/admin.token"
