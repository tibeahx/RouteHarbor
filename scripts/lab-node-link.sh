...
docker run --rm --network none --cap-add NET_ADMIN --cap-add NET_RAW --read-only --tmpfs /tmp:rw,nosuid,nodev,size=32m \
  -e OPENRHP_NODE_LINK_LAB=1 -v "$PWD/test-results:/work/test-results:ro" \
  --entrypoint /work/test-results/node-link.test openrhp-path-lab:local -test.run '^TestLinuxVethNodeLinkTelemetry$' -test.v
