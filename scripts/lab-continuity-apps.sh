...
cd "$(dirname "$0")/.."
: "${GO:=go}"
: "${OPENRHP_CONTINUITY_APP_IMAGE:=openrhp-continuity-app-lab:local}"
mkdir -p test-results/continuity
if ! docker image inspect "$OPENRHP_CONTINUITY_APP_IMAGE" >/dev/null 2>&1; then
  docker build -t "$OPENRHP_CONTINUITY_APP_IMAGE" docker/lab-continuity-apps
fi
lab_arch=$(docker image inspect "$OPENRHP_CONTINUITY_APP_IMAGE" --format '{{.Architecture}}')
...
docker run --rm --network none --read-only --cap-drop ALL --cap-add NET_ADMIN \
  ...
  --entrypoint /tests/applications.test -e OPENRHP_CONTINUITY_APP_LAB=1 \
  "$OPENRHP_CONTINUITY_APP_IMAGE" -test.v -test.run '^TestLinuxContinuityApplications$' -test.timeout 3m
