...
: "${ROUTEHARBOR_DISPATCHER_LAB_MEMORY:=512m}"
lab_build="$(mktemp -d "${TMPDIR:-/tmp}/routeharbor-dispatcher-worker.XXXXXX")"
...
scale_fixture=
if [ -n "${ROUTEHARBOR_DISPATCHER_SCALE_DOMAINS:-}" ]; then
 cp "$ROUTEHARBOR_DISPATCHER_SCALE_DOMAINS" "$lab_build/domains.lst"
 chmod 0644 "$lab_build/domains.lst"
 scale_fixture=/lab/domains.lst
fi
docker run --rm --network none --privileged --memory "$ROUTEHARBOR_DISPATCHER_LAB_MEMORY" \
 --memory-swap "$ROUTEHARBOR_DISPATCHER_LAB_MEMORY" --pids-limit 256 \
 --mount "type=bind,source=$lab_build,target=/lab,readonly" \
 --env ROUTEHARBOR_NET_LAB=1 --env ROUTEHARBOR_DISPATCHER_WORKER_LAB=1 \
 --env "ROUTEHARBOR_DISPATCHER_SCALE_DOMAINS=$scale_fixture" \
 --env "ROUTEHARBOR_DISPATCHER_SCALE_WORKERS=${ROUTEHARBOR_DISPATCHER_SCALE_WORKERS:-1}" \
 ...
 lab_status=0
 /usr/libexec/routeharbor-lab-helper.test -test.v -test.run "^TestLinuxDispatcherWorkerRPCPrivilegesDNSAndOwnerCleanup$" -test.timeout 150s || lab_status=$?
