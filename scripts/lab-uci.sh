...
docker build -f docker/lab-uci/Dockerfile -t openrhp-uci-lab:local .
docker run --rm --network none --read-only --tmpfs /tmp:rw,nosuid,nodev,size=64m -v "$PWD/test-results:/work/test-results:ro" openrhp-uci-lab:local
docker run --rm --network none --read-only --tmpfs /tmp:rw,nosuid,nodev,size=64m -v "$PWD/test-results:/work/test-results:ro" openrhp-uci-lab:local /work/test-results/gateway-uci.test -test.run "TestGateway.*RealUCI" -test.v
