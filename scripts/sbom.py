...
namespace = 'https://github.com/tibeahx/RouteHarbor/sbom/' + commit
if dirty:
...
version = '0.1.0-dev'
relationships = [
    {'spdxElementId': 'SPDXRef-DOCUMENT', 'relationshipType': 'DESCRIBES', 'relatedSpdxElement': 'SPDXRef-RouteHarbor'},
    {'spdxElementId': 'SPDXRef-RouteHarbor', 'relationshipType': 'DEPENDS_ON', 'relatedSpdxElement': 'SPDXRef-Go'},
]
files = []
artifact_dir = Path(os.environ.get('OUT', str(root / 'dist')))
for artifact in sorted(artifact_dir.glob('routeharbor*-linux-*')):
    ...
    relationships.extend([
        ...
        {'spdxElementId': file_id, 'relationshipType': 'GENERATED_FROM', 'relatedSpdxElement': 'SPDXRef-RouteHarbor'},
    ])
sbom = {
    'spdxVersion': 'SPDX-2.3', 'dataLicense': 'CC0-1.0', 'SPDXID': 'SPDXRef-DOCUMENT',
    'name': 'RouteHarbor core ' + version,
    'documentNamespace': namespace,
    'creationInfo': {'creators': ['Tool: RouteHarbor scripts/sbom.py'],
                     'created': datetime.datetime.now(datetime.timezone.utc).strftime('%Y-%m-%dT%H:%M:%SZ')},
    'packages': [
        {'name': 'RouteHarbor', 'SPDXID': 'SPDXRef-RouteHarbor', 'versionInfo': version,
         'downloadLocation': 'NOASSERTION' if dirty else 'git+https://github.com/tibeahx/RouteHarbor.git@' + commit,
