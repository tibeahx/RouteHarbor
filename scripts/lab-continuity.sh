...
: "${ROUTEHARBOR_CONTINUITY_SWITCHES:=1000}"
...
docker run --rm --network none --read-only --cap-drop ALL --cap-add NET_ADMIN \
  ...
  -e ROUTEHARBOR_CONTINUITY_LAB=1 -e "ROUTEHARBOR_CONTINUITY_SWITCHES=$ROUTEHARBOR_CONTINUITY_SWITCHES" \
  "$ROUTEHARBOR_CONTINUITY_LAB_IMAGE" -test.v -test.run '^TestLinuxContinuityQualification$' -test.timeout 15m
