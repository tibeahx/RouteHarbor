...
recipe="$sdk_dir/package/routeharbor"
if [ -e "$recipe" ]; then
 [ -f "$recipe/.routeharbor-owned" ] || { echo 'Refusing to replace an existing foreign SDK package directory.' >&2; exit 1; }
 rm -rf "$recipe"
fi
mkdir -p "$recipe"
printf '%s\n' 'RouteHarbor SDK staging directory' > "$recipe/.routeharbor-owned"
cp -R "$source_dir/packaging/openwrt/routeharbor/." "$recipe/"
...
if [ ! -f "$sdk_dir/.config" ]; then
 printf '%s\n' '# CONFIG_ALL is not set' '# CONFIG_ALL_KMODS is not set' '# CONFIG_ALL_NONSHARED is not set' \
  'CONFIG_PACKAGE_routeharbor=m' 'CONFIG_PACKAGE_routeharbor-guard=m' 'CONFIG_PACKAGE_routeharbor-node=m' \
  'CONFIG_PACKAGE_routeharbor-continuity=m' 'CONFIG_PACKAGE_routeharbor-conntrack=m' 'CONFIG_PACKAGE_routeharbor-sing-box=m' 'CONFIG_PACKAGE_routeharbor-xray=m' > "$sdk_dir/.config"
fi
...
make -C "$sdk_dir" package/routeharbor/clean NO_DEPS=1 V=s
...
make -C "$sdk_dir" package/routeharbor/compile NO_DEPS=1 V=s CONFIG_PACKAGE_routeharbor-conntrack=m CONFIG_PACKAGE_routeharbor-continuity=m ROUTEHARBOR_SOURCE="$source_dir" ROUTEHARBOR_GO="$go_binary"
# Inspect the actual seven current IPKs. In particular, SDK stripping must not
