...
if [ ! -f "$sdk_dir/.config" ]; then
 printf '%s\n' '# CONFIG_ALL is not set' '# CONFIG_ALL_KMODS is not set' '# CONFIG_ALL_NONSHARED is not set' \
  ...
  'CONFIG_PACKAGE_openrhp-continuity=m' 'CONFIG_PACKAGE_openrhp-conntrack=m' 'CONFIG_PACKAGE_openrhp-sing-box=m' 'CONFIG_PACKAGE_openrhp-xray=m' > "$sdk_dir/.config"
fi
...
make -C "$sdk_dir" package/openrhp/compile NO_DEPS=1 V=s CONFIG_PACKAGE_openrhp-conntrack=m CONFIG_PACKAGE_openrhp-continuity=m OPENRHP_SOURCE="$source_dir" OPENRHP_GO="$go_binary"
