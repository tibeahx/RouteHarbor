...
CGO_ENABLED=0 GOOS=linux GOARCH="$go_arch" "$GO" test -c -o test-results/release/release-cli.test ./cmd/routeharbor-release
...
for binary in release-cli release-core; do
 docker run --rm --network none --read-only --cap-drop ALL \
  ...
  --entrypoint /usr/bin/timeout routeharbor-path-lab:local -k 5s 150s \
